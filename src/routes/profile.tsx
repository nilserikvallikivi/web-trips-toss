...
function Inner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: pv } = await (supabase as any)
        .from("presence_privacy_settings")
        .select("visibility")
        .eq("user_id", user.id)
        .maybeSingle();
  ...
  const savePresencePrivacy = async (v: string) => {
    ...
    await (supabase as any)
      .from("presence_privacy_settings")
      .upsert({ user_id: user.id, visibility: v, updated_at: new Date().toISOString() });
    ...
    <div className="max-w-xl space-y-6">
      ...
      <form onSubmit={save} className="space-y-4 rounded-xl border border-border bg-card p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-2">
            ...
            <Select value={form.gender} onValueChange={(v) => setForm({ ...form, gender: v })}>
              ...
              <SelectContent>
                {["male", "female", "other", "unspecified"].map((x) => (
          ...
          <div className="space-y-2">
            ...
            <Select value={form.skill_level} onValueChange={(v) => setForm({ ...form, skill_level: v })}>
              ...
              <SelectContent>
                {["beginner", "intermediate", "advanced", "league"].map((x) => (
                  <SelectItem key={x} value={x}>
                    {x}
                  </SelectItem>
                ))}
        ...
        <div className="space-y-2">
          ...
          <Select
            value={form.dominant_hand || "unset"}
            onValueChange={(v) => setForm({ ...form, dominant_hand: v === "unset" ? "" : v })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
      ...
      <div className="rounded-xl border border-destructive/30 bg-card p-6 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          Deactivate or delete your account. If you have match, ranking, or registration history, your account will be
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                You will be signed out. If you have historical data, your name will be anonymized but match, ranking,
                and registration history is preserved. This cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
